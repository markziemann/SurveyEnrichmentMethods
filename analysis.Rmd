...
```
journalabbrev <- medline[grep("Title(s):",medline)]


jdf <- data.frame(journaltitle,journalabbrev)

m <- merge(jmetrics,jdf,by.x="Title",by.y="journaltitle")
dim(m)
mm <- m[,c("journalabbrev","SJR")]
dim(mm)

dim(x)
xm <- merge(x,mm,by.x="Journal",by.y="journalabbrev")
dim(xm)
tail(xm)

dim(x)
length(which(x$Journal %in% mm$journalabbrev))

x[which(! x$Journal %in% mm$journalabbrev),"Journal"]
