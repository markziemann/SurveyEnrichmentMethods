...
par(mar=c(5,12,3,1))
barplot(tail(res,20),horiz=TRUE,las=1,cex.names = 1, xlab="no. analyses",
        main = "Assumptions violated", xlim=c(0,100))
text(y = (1:length(res)*1.2)-0.75  , x = res+100, label = res, pos = 3, cex = 1, col = "black")
grid()
dev.off()
...
plot(jjres$SJR,jjres$scores,xlab="SJR",ylab="mean score",main="Association of mean analysis score with journal metrics")
abline(mylm2,col="red")

png("images/mean_sjr1.png")
par(mar=c(5,5,3,1))
plot(jjres$SJR,jjres$scores,xlab="SJR",ylab="mean score",main="Association of mean analysis score with journal metrics")
abline(mylm2,col="red")
dev.off()
