...
To demonstrate the effect of misusing gene set analysis, a publicly available RNA-seq dataset (accession SRP128998) was downloaded from DEE2 (Ziemann et al, 2019).
This data consists of hepatocytes grown in culture with and without glucose first described by Felisbino et al (2021) and here we are using the comparison of cells grown in low glucose control condition compared to high glucose as an example.
Transcript level counts were aggregated to genes using the getDEE2 R package. 
Next, genes with an average of 10 reads per sample were discarded.
Differential expression statistical analysis was conducted with DESeq2 (Love et al, 2014).
...
FCS was performed using the mitch R package, which uses a rank-ANOVA statistical test (Kaspi and Ziemann 2020).
ORA analysis was performed using the clusterProfiler R package enricher function that implements a hypergeometric test (Yu et al, 2012).
...
Analyses were conducted in R version 4.1.
...
Out of 39297 genes in the annotation set, 15635 were above the detection threshold.
Statistical analysis revealed 3472 differentially expressed genes with 1560 up-regulated and 1912 down-regulated (FDR<0.05) due to glucose exposure.
...
For differential expression analysis, combining up- and down-regulated gene lists for ORA is considered poor practice (Hong et al, 2014).
Our own tests indicate that combining up- and down-regulated gene lists prior to ORA dramatically reduces the statistical strength when the proportion of differentially expressed genes is high.
